...
Place this file at: G:/Featherflow/EfficientNet-PyTorch/train.py
...
def train_model(model, dataloaders, dataset_sizes, criterion, optimizer,
    ...
    scaler = GradScaler("cuda")     # ← fixed: explicit 'cuda' device
        ...
        for phase in ["train", "val"]:
            ...
            for inputs, labels in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(phase == "train"):
                    with autocast("cuda"):      # ← fixed: explicit 'cuda' device
...
def main():
    ...
    if device.type != "cuda":
        ...
        raise SystemExit("Please install CUDA PyTorch first before training.")

    print(f"GPU: {torch.cuda.get_device_name(0)}")
    torch.backends.cudnn.benchmark = True   # ← speeds up training on GPU
